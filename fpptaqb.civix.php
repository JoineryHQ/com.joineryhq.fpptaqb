 ...
 * extension.
 */
class CRM_Fpptaqb_ExtensionUtil {
  const SHORT_NAME = 'fpptaqb';
  const LONG_NAME = 'com.joineryhq.fpptaqb';
  const CLASS_PREFIX = 'CRM_Fpptaqb';
...
spl_autoload_register('_fpptaqb_civix_class_loader', TRUE, TRUE);

function _fpptaqb_civix_class_loader($class) {
  if ($class === 'CRM_Fpptaqb_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_Fpptaqb_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\Fpptaqb\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\Fpptaqb\\') === 0) {
 ...
 * @return bool
 */
function _fpptaqb_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _fpptaqb_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _fpptaqb_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _fpptaqb_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _fpptaqb_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _fpptaqb_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _fpptaqb_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {

  ...
  public function run() {
    ...
    switch ($type) {
      ...
      case 'cm':
        switch ($itemaction) {
          case 'load':
            CRM_Utils_System::setTitle(E::ts('Load Credit Memo'));
            try {
              $creditmemoLoad = _fpptaqb_civicrmapi('FpptaqbStepthruCreditmemo', 'load', ['id' => $id]);
              $content = ($creditmemoLoad['values']['text'] ?? NULL);
              $this->assign('content', $content);
            }
            catch (API_Exception | CiviCRM_API3_Exception $e) {
              $this->assign('apiError', $e->getMessage());
          ...
          case 'unhold':
            ...
            $msg = E::ts('Credit memo %1 has been un-held.', [1 => $id]);
            CRM_Core_Session::setStatus($msg, 'Success', 'success');
        ...
        break;
      default:
        CRM_Core_Error::statusBounce('Invalid type; must be "inv", "pmt", or "cm".');
    }

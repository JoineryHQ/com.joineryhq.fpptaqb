...
/**
 * FpptaqbBatchSyncCreditmemos.Process API specification (optional)
 ...
 */
function _civicrm_api3_fpptaqb_batch_sync_creditmemos_Process_spec(&$spec) {
}

/**
 * FpptaqbBatchSyncCreditmemos.Process API
 *
 * @param array $params
 *
 * @return array
 ...
 *
 */
function civicrm_api3_fpptaqb_batch_sync_creditmemos_Process($params) {
  $syncedIds = $heldIds = [];
  try {
    ...
    while ($nextId = CRM_Fpptaqb_Utils_Creditmemo::getReadyToSyncIdNext()) {
      try {
        ...
        // the creditmemo, it would get us the actual hash, which would be valid,
        ...
        $sync = _fpptaqb_civicrmapi('FpptaqbStepthruCreditmemo', 'sync', [
          'id' => $nextId,
          'hash' => $hashBypass,
        ]);
        $syncedIds[] = $nextId;
        ...
        // if anything goes wrong, use the hold api to place this creditmemo on hold,
        // and give the log_reason as whatever is in the $e->errorMessage().
        // Then move on to the next creditmemo.
        $holdReason = $e->getMessage();
        try {
          $hold = _fpptaqb_civicrmapi('FpptaqbStepthruCreditmemo', 'hold', [
           'id' => $nextId,
           'log_reason' => 'Error encountered in sync: '. $holdReason,
         ]);
        }
        catch (Exception $e) {
          throw new CRM_Fpptaqb_Exception("Error in trying to hold credit memo $nextId (hold reason was: $holdReason): " . $e->getMessage(), '500');
        }
        $heldIds[] = $nextId;
      }
    }
    return civicrm_api3_create_success('synced: '. json_encode($syncedIds) .  '; held: ' . json_encode($heldIds), $params, 'FpptaqbBatchSyncCreditmemos', 'Process');
  }
  catch (Exception $e) {
